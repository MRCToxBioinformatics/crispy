...
This notebook generates QC plots from the guide RNA (gRNA) counts table ('quant.dir/all_samples.tsv') and the read alignments to the gRNA sequences.
...
experimental_design <- read.delim(here('experimental_design.tsv'), sep='\t', row.names=NULL) %>%
  tibble::column_to_rownames('sample')

if(identical(colnames(experimental_design), c('concentration'))){
  crispr_type <- 'drug'
} else if(length(setdiff(colnames(experimental_design), c('condition', 'sort')))==0){
  crispr_type <- 'sorted'
} else{
  crispr_type <- 'other'
  warning(paste('Columns in experimental_design.tsv must either be "condition" and "sort"',
                '(pooled sorted screen) or "concentration" (drug viability screen).',
                'Falling back on using the first two columns in experimental_design.tsv as the plotting aesthetics'))
}
...
# Errors relative to expected gRNA sequences

Using the alignments for reads to the gRNA sequences, we can extract the number of errors in the read sequence.
...
ggsave(here('qc_plots.dir/error_tallies.pdf'), width=4, height=4)
ggsave(here('qc_plots.dir/error_tallies.png'), width=4, height=4)  
...
ggsave(here('qc_plots.dir/gini_coefficients.pdf'), width=3, height=3)
ggsave(here('qc_plots.dir/gini_coefficients.png'), width=3, height=3)
```
...
ggsave(here('qc_plots.dir/gRNAs_per_gene.pdf'), width=4, height=4)
ggsave(here('qc_plots.dir/gRNAs_per_gene.png'), width=4, height=4)
```
...
```{r plot guides per sample, message=FALSE, warning=FALSE}
...
ggsave(here('qc_plots.dir/gRNAs_per_sample.pdf'), width=4, height=4)
ggsave(here('qc_plots.dir/gRNAs_per_sample.png'), width=4, height=4)

```

```{r plot read depth per guide, message=FALSE}
if(crispr_type=='sorted'){
  p <- gRNA_counts %>%
    pivot_longer(cols=-c(gene, sgRNA)) %>%
    merge(experimental_design, by.x='name', by.y='row.names') %>%
    filter(!is.na(condition)) %>%
    ggplot(aes(value, group=name, colour=factor(condition), linetype=factor(sort))) +
    geom_density() +
    scale_x_log10() +
    my_theme(border=FALSE, base_size=15, base_family='sans') +
    xlab('Reads') +
    ylab('density') +
    scale_colour_manual(values=get_my_colours(7), name='Condition') +
    scale_linetype_discrete(name='Sort') +
    ggtitle('Reads per gRNA')
  
  print(p)
  
} else if(crispr_type=='drug'){
  p <- gRNA_counts %>%
    pivot_longer(cols=-c(gene, sgRNA)) %>%
    merge(experimental_design, by.x='name', by.y='row.names') %>%
    filter(!is.na(concentration)) %>%
    ggplot(aes(value, group=name, colour=factor(concentration))) +
    geom_density() +
    scale_x_log10() +
    my_theme(border=FALSE, base_size=15, base_family='sans') +
    xlab('Reads') +
    ylab('density') +
    scale_colour_manual(values=c('grey', get_my_colours(7)), name='Concentration') +
    ggtitle('Reads per gRNA')
  
  print(p)
  
} else if(crispr_type=='other'){
  colour_col = colnames(experimental_design)[1]
  p <- gRNA_counts %>%
    pivot_longer(cols=-c(gene, sgRNA)) %>%
    merge(experimental_design, by.x='name', by.y='row.names') %>%
    filter(!is.na(!!sym(colour_col))) %>%
    ggplot(aes(value, group=name, colour=factor(!!sym(colour_col)))) +
    geom_density() +
    scale_x_log10() +
    my_theme(border=FALSE, base_size=15, base_family='sans') +
    xlab('Reads') +
    ylab('density') +
    scale_colour_manual(values=c('grey', get_my_colours(7)), name=colour_col) +
    ggtitle('Reads per gRNA')
  
  if(ncol(experimental_design)>1){
    linetype_col = colnames(experimental_design)[2]
    p <- p + aes(linetype=factor(!!sym(linetype_col))) +
      scale_linetype_discrete(name=linetype_col)
  }
...
if(crispr_type=='sorted'){
  ...
  p <- all_resampled_guides_per_sample %>%
    ...
    ggplot(aes(as.numeric(depth), gRNAs, group=sample, colour=factor(condition), linetype=factor(sort))) +
    geom_line() +
  ...
  p <- all_resampled_guides_per_sample %>%
    mutate(reads_threshold=sprintf('>%s reads', reads_threshold)) %>%
    merge(experimental_design, by.x='sample', by.y='row.names') %>%
    ggplot(aes(as.numeric(depth), gRNAs, group=sample, colour=factor(concentration))) +
    geom_line() +
    facet_grid(reads_threshold~.) +
    my_theme(border=FALSE, base_family='sans', base_size=15) +
    theme(strip.background=element_blank()) +
  ...
  p <- all_resampled_guides_per_sample %>%
    mutate(reads_threshold=sprintf('>%s reads', reads_threshold)) %>%
    merge(experimental_design, by.x='sample', by.y='row.names') %>%
    ggplot(aes(as.numeric(depth), gRNAs, group=sample, colour=factor(!!sym(colour_col)))) +
    geom_line() +
    facet_grid(reads_threshold~.) +
    my_theme(border=FALSE, base_family='sans', base_size=15) +
    theme(strip.background=element_blank()) +
...
ggsave(here('qc_plots.dir/sequencing_depth.pdf'), width=5, height=7)
ggsave(here('qc_plots.dir/sequencing_depth.png'), width=5, height=7)
```
...
```{r plot correlation matrix, fig.height=5, fig.width=5, message=FALSE}

mat <- cor(cts)
pheatmap(mat, cellwidth=10, cellheight=10)
pheatmap(mat, cellwidth=10, cellheight=10, filename=here('qc_plots.dir/correlations.pdf'), width=4, height=4)
pheatmap(mat, cellwidth=10, cellheight=10, filename=here('qc_plots.dir/correlations.png'), width=4, height=4)
```
# PCA
...
if(crispr_type=='sorted'){
  ...
  p <- DESeq2::plotPCA(vst, intgroup=colnames(experimental_design_complete), ntop=nrow(vst)) +
    ...
    aes(shape=factor(sort), fill=factor(condition), colour=factor(condition)) +
    geom_point(size=5) +
    scale_fill_manual(values=get_my_colours(7), name='Condition') +
    scale_colour_manual(values=get_my_colours(7), name='Condition') +
    scale_shape_discrete(name='Sort') +
  ...
  dds <- DESeqDataSetFromMatrix(countData = cts[,rownames(experimental_design)],
                                colData = experimental_design,
                                design = as.formula(sprintf('~ %s', paste(colnames(experimental_design), collapse='+'))))
  
  vst <- vst(dds, blind=FALSE)
  
  p <- DESeq2::plotPCA(vst, intgroup=colnames(experimental_design), ntop=nrow(vst)) +
    my_theme(border=FALSE, base_family='sans') +
    aes(fill=factor(concentration), colour=factor(concentration)) +
    geom_point(colour='grey30', pch=21, size=5) +
    scale_shape_manual(values=21:25) +
    scale_fill_manual(values=c('grey', get_my_colours(7)), name='Concentration') +
    scale_colour_manual(values=c('grey', get_my_colours(7)), guide='none') +
    guides(fill = guide_legend(override.aes = list(pch=21) ) )
  ...
  condition1 = colnames(experimental_design)[1]
  
  if(ncol(experimental_design)>1){
    condition2 = colnames(experimental_design)[2]  
    
    dds <- DESeqDataSetFromMatrix(countData = cts[,rownames(experimental_design)],
                                colData = experimental_design,
                                design = as.formula(sprintf('~ %s', paste(c(condition1, condition2), collapse='+'))))
  
    vst <- vst(dds, blind=FALSE)
  
    p <- DESeq2::plotPCA(vst, intgroup=colnames(experimental_design), ntop=nrow(vst)) +
      my_theme(border=FALSE, base_family='sans') +
      aes(fill=factor(!!sym(condition1)), colour=factor(!!sym(condition1)), shape=factor(!!sym(condition2))) +
    geom_point(size=5) +
    scale_fill_manual(values=c('grey', get_my_colours(7)), name=condition1) +
    scale_shape_discrete(name=condition2) +
    scale_colour_manual(values=c('grey', get_my_colours(7)), guide=FALSE) +
    guides(fill = guide_legend(override.aes = list(pch=21) ) )
  }
