...
```{r}
error_counts <- read.table('~/projects/09_CRIPSR_screen_Dimitris/pipeline_runs/test_tech_rep_sep/bowtie.dir/all_errors.tsv', sep='\t', header=TRUE)
error_counts$errors[is.na(error_counts$errors)] <- 'Unmapped'

experimental_design <- read.delim(here('~/projects/09_CRIPSR_screen_Dimitris/pipeline_runs/test_tech_rep_sep/experimental_design.tsv'), sep='\t', row.names=1)
```

# Errors
```{r plot error tallies}
...
```{r plot Gini coefficient}
gRNA_counts_exp_only %>% apply(MARGIN=2, Gini) %>% data.frame() %>%
...
```{r plot correlation matrix, fig.height=7, fig.width=7}
