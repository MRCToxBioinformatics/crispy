...
if(identical(colnames(experimental_design), c('concentration'))){
  crispr_type <- 'drug'
} else if(identical(colnames(experimental_design), c('condition', 'sort'))){
  crispr_type <- 'sorted'
} else{
  stop(paste('Columns in experimental_design.tsv must either be "condition" and "sort"',
...
if(crispr_type=='sorted'){
  p <- gRNA_counts %>%
    ...
    xlab('Reads') +
    ylab('density') +
    scale_colour_manual(values=get_my_colours(3), name='Sort') +
...
if(crispr_type=='sorted'){
  ...
  p <- DESeq2::plotPCA(vst, intgroup=colnames(experimental_design_complete), ntop=nrow(vst)) +
    ...
    geom_point(colour='grey30', size=5) +
  ...
  experimental_design_complete <- experimental_design
